...
| Couldn't load / offline | ✅ 🔨 | **Built (2026-08-26).** Says it failed and offers **Reload**, rather than production's approach of timing out after 5–6s and presenting a *guess* as fact ("roll call is closed", "no gear") — which confidently misinforms anyone on a bad connection. **Offline is the same component with different words** ("You're offline / Check your connection"), keyed on `navigator.onLine`: "reload" is useless advice with no signal, and "check your connection" is insulting when the connection is fine. Two sizes — **section** for a partial failure (gear fails, roster stays usable) and **page** for a total one, since without the roster we know nothing about the screen, not even whether you're on the list. Preview: `?error=1` · `?error=offline` · `?error=gear` |
| ↳ App-level wait | ⬜ | Production replaces the whole page with "Loading…" while the session doc arrives. Not yet designed for the redesign — the skeletons cover the screens' own content, not the shell |
