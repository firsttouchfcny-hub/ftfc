...
| ↳ **Non-`+1` numbers are refused** | ⬜ ❓ | **Open question, inherited from production (verified 2026-08-24).** Every entry point funnels through `toE164US`, which accepts only the **+1 range** — so Canada and the Caribbean already work, but +52 / +55 / +57 / +44 do not. Verification is mandatory (`App.jsx:409`), so an excluded number means **no account at all**; production's only answer is an admin vouching by name (`markVerifiedByName`), which the redesign has no equivalent of. For a Brooklyn club this is plausible, not hypothetical. **Deferred (2026-08-24):** not changing production logic for now, so the flow simply drops the "+1" box rather than pretending to offer a choice. Revisit as either widening the normalizer or giving the flow a visible "ask an admin" path |
...
| ↳ **Still shows a "+1" box** | ⬜ ❓ | Edit profile (`3233:12568`) keeps the read-only "+1" field that the account flow just dropped. Left alone deliberately — it's a different screen with its own frame and wasn't in scope — but the two now disagree. Worth settling |
