...
| State | Status | Notes |
|---|---|---|
| Loading — roster & gear | ✅ 🔨 | **Built (2026-08-26):** skeletons, not spinners — the shape of what's coming is known, so placeholders reassure rather than just spin. Sized to the real elements (rows 325×48, gear tiles 48×72, 24px gap — measured against both) so nothing shifts on arrival. Gear deliberately shows **no "+"**: an unloaded tile and an unclaimed one are the same picture, so a plus would invite taking a set that's already gone. Pulse, not shimmer; `prefers-reduced-motion` keeps the shape and drops the animation. The **headline waits too** (2026-08-26) — "You're in" / "2nd in bench" / "No game" is a *claim* that depends on the roster still arriving, and its placeholder occupies the full 36px line so nothing shifts. Date, time and place never wait: they come from the clock and fixed copy. Preview: `?loading=1` |
...
| ↳ **Sign-up count on the roll-call screen** | ⬜ 🎨 | **Wanted (2026-08-25).** Production shows the full roster on the same screen as the sign-up buttons, so you can see how full the game is — and whether Match 2 is cancelled — *before* you tap. The redesign put the roster behind sign-up, so that context is gone at the moment of decision. Add a count (or similar) to the roll-call screen to close the gap the split created. **Cristian is working out the approach** — no frame yet, and not started. Would sit in the existing `StatusBadge`, under the game details, where the waiting variant puts its countdown |
